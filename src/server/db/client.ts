...
function getDatabaseConfig() {
  const containerConfigPath = '/alloc/config.json';
  const localConfigPath = './config.json';

  let configPath = containerConfigPath;

  if (!existsSync(configPath)) {
    configPath = localConfigPath;
    if (!existsSync(configPath)) {
      throw new Error(
        `Database configuration file not found. Expected ${containerConfigPath} or ${localConfigPath}`
      );
    }
  }
